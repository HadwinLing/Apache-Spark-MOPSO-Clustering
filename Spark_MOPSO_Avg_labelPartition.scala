...
object Main {
  ...
  * */
  def main(args: Array[String]): Unit = {
    ...
    val outputHDFS: String = "hdfs://211.69.243.34:9000/linghuidong/output/" + dataName + "/labelPartition"
    ...
    val conf = new SparkConf().setAppName("Spark_MOPSO_Avg_labelPartition")
    ...
    var allDataWithKRDDRandomPartition: RDD[(Array[Double], Int)] = HashPartitionerValue // 再分区
    ...
    // 进行常规实验
    val allArchive: Array[(Array[Array[Double]], Array[Double], Array[Double])] = Archive
    ...
    allDataWithKRDDRandomPartition = allDataWithKRDDRandomPartition.repartition(numberOfWorker * 40).persist() // 数据打乱分区,
    // 对全部结果进行计算
